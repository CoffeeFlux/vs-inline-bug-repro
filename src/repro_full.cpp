// MSVC ICE reproducer — full-fidelity version.
//
// Mirrors the code shape in Aegisub's libaegisub/lua/modules/lfs.cpp before
// https://github.com/TypesettingTools/Aegisub/pull/666 hoisted the status()
// call out of the switch condition to dodge an internal compiler error in
// MSVC 19.44 (VS 2022 17.14, toolset 14.44).
//
// The `path` class below is copied nearly verbatim from Aegisub's
// libaegisub/include/libaegisub/fs.h (BSD-licensed). It derives from
// std::filesystem::path and converts UTF-8 narrow strings via char8_t.
//
// Expected: "fatal error C1001: Internal compiler error" on affected
// toolsets when compiling the switch condition in get_mode() below.
// workaround.cpp is identical except the status() call is hoisted into a
// local variable, and is expected to compile everywhere.
